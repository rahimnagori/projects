...
class Common_Model extends CI_Model
{
  ...
  public function getPageData()
  {
    ...
    return array(
        ...
        'cart' => (!$cart || empty($cart)) ? 0 : count($cart),
        'pageTitle' => $this->load_page_title(),
        'pageMetaDescription' => $this->load_page_meta_tags()
    );
  }

  private function load_page_title(){
    $titles = [
      "LED Bulbs, Lights, and Street Lights | " .$this->config->item('PROJECT'), 
      "High-Quality LED Bulbs and Lighting Solutions | Shop Now | " .$this->config->item('PROJECT'), 
      "Energy-Efficient LED Lights for Homes and Businesses | " .$this->config->item('PROJECT'), 
      "Brighten Your Space with LED Bulbs and Ceiling Panel Luminaries | " .$this->config->item('PROJECT'), 
      "Top-Quality Street Lights and LED Lighting Fixtures | " .$this->config->item('PROJECT'), 
      "Illuminate Your World: LED Bulbs, Lights, and More  | " .$this->config->item('PROJECT'), 
      "Modern LED Lighting Solutions for Every Need | " .$this->config->item('PROJECT'), 
      "Environmentally Friendly LED Bulbs and Lighting Products | " .$this->config->item('PROJECT'), 
      "Find the Perfect LED Lights and Street Lights | " .$this->config->item('PROJECT'), 
      "Leading Supplier of LED Bulbs, Lights, and Street Lighting | " .$this->config->item('PROJECT')];
    $titleLength =  count($titles) - 1;

    return $titles[rand(0, $titleLength)];
  }

  private function load_page_meta_tags(){
    $metas = [
      "Discover a wide range of LED bulbs, lights, and street lights at " .$this->config->item('PROJECT') .". Our energy-efficient lighting solutions are perfect for homes, businesses, and public spaces. Shop now for the latest in LED technology.",

      "Light up your life with our premium LED bulbs and lighting fixtures. Explore our collection of energy-saving options, including ceiling panel luminaries and street lights. Find the perfect lighting solution at " .$this->config->item('PROJECT') .".",

      "Looking for high-quality LED bulbs and lights? Look no further! At " .$this->config->item('PROJECT') .", we offer a diverse selection of eco-friendly lighting solutions, including street lights and ceiling panel luminaries. Transform your space with us today.",

      "Illuminate your surroundings with our cutting-edge LED lighting products. From stylish LED bulbs to durable street lights, we provide innovative solutions for all your lighting needs. Explore our range at " .$this->config->item('PROJECT') .".",

      "Upgrade to energy-efficient LED lighting with " .$this->config->item('PROJECT') .". We specialize in LED bulbs, lights, and street lights that are not only eco-friendly but also cost-effective. Enhance your environment with our superior lighting solutions.",

      "At " .$this->config->item('PROJECT') .", we offer a variety of LED lighting options, including LED bulbs, lights, and street lights designed for optimal performance. Explore our range and experience the difference of quality lighting.",

      "Create a vibrant ambiance with our state-of-the-art LED bulbs and lights. From elegant ceiling panel luminaries to reliable street lights, we provide exceptional lighting solutions. Find your perfect match at " .$this->config->item('PROJECT') .".",

      "Brighten your space with our range of LED bulbs, lights, and ceiling panel luminaries. Our energy-efficient lighting products are not only stylish but also long-lasting. Discover the ideal lighting solutions at " .$this->config->item('PROJECT') .".",

      "Light up your world with our premium LED lighting products. We offer a diverse selection of LED bulbs, lights, street lights, and more. Trust " .$this->config->item('PROJECT') ." for top-quality, environmentally conscious lighting solutions.",

      "Enhance visibility and aesthetics with our LED lighting solutions. Whether you need LED bulbs for your home or street lights for a community project, " .$this->config->item('PROJECT') ." has you covered. Explore our range today."
    ];
    $metaLength =  count($metas) - 1;
    
    return $metas[rand(0, $metaLength)];
  }
}